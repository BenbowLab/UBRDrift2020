...
SturgeonCTU<-ggplot(AllData,aes(x=CTUSturgeon,y=SturgeonConc))+geom_point(size=0.5)+xlab("Cumulative Temperature Units")+facet_wrap(~Year)+ylab(expression(Abundance~Per~100~m^3~Drift))+
theme(axis.text.x = element_text(angle = 45, hjust = 1))
SturgeonCTU
head(AllData)

dev.off()
tiff("Figures/SturgeonCTUBase5.8.tiff", width = 84, height = 84, units = 'mm', res = 1200)
SturgeonCTU
dev.off()
...
##########
head(AllData)
Sturgeon<-ggplot(AllData,aes(CTUSturgeon,SturgeonConc))+geom_point(color="#000000",shape=15,size=1)+theme(axis.title.y=element_blank())+xlab("Cumulative Temperature Units (CTU)")+ scale_y_continuous(breaks=c(0,25,50,75,100),limits=c(0,100))
Invert<-ggplot(AllData,aes(CTUSturgeon,DriftInvertConc))+geom_point(color="#E69F00",shape=17,size=1)+theme(axis.title.x=element_blank(),axis.title.y=element_blank())
Cato<-ggplot(AllData,aes(CTUSturgeon,SuckerConc))+geom_point(color="#0072B2",shape=16,size=1)+theme(axis.title.x=element_blank(),axis.title.y=element_blank(),axis.text.y = element_text(size = 7))
...
dev.off()
tiff("Figures/Fig1Oct2020.tiff", width = 174, height = 190, units = 'mm', res = 1200)
...
ShannonSubset2<-subset(ShannonSubset, Q!= "NA")
...
SuckerGAMPlot<-ggplot(ShannonSubset,aes(CTUSturgeon,SuckerConc))+geom_point(color="darkgrey")+geom_line(data=PredictedDataFrame,size=1.5)+  
  ...
  xlab("Cumulative Temperature Units")+ylab(expression(Larval~Sucker~Per~100~m^3~Drift))+theme(axis.title.x=element_text(size = 8))#+ylim(NA, 1000)
SuckerGAMPlot
...
SuckerGAMPlot2<-ggplot(ShannonSubset,aes(percillum,SuckerConc))+geom_point(color="darkgrey")+geom_line(data=PredictedDataFrame,size=1.5)+  
  geom_line(data = PredictedDataFrame, aes(y = LowerCI), size = .75,linetype="dashed")+geom_line(data = PredictedDataFrame,aes(y=UpperCI),size=0.75,linetype="dashed")+
  xlab("Percent Illumination (%)")+ylab(expression(Larval~Sucker~Per~100~m^3~Drift))+theme(axis.title.x=element_text(size = 8))#+ylim(NA, 1000)
SuckerGAMPlot2
...
appraise(SturgeonAbuLogDischarge)
AICctab(SturgeonAbuLog0,SturgeonAbuLog1,SturgeonAbuLog2,SturgeonAbuLog3,SturgeonAbuLog4,SturgeonAbuLog5,SturgeonAbuLog6,SturgeonAbuLog7,SturgeonAbuLog8,SturgeonAbuLog9,
        weights=T)
summary(SturgeonAbuLog1)
appraise(SturgeonAbuLog1)
SturgeonAbuLog7$aic
#Gamma Models
...
cor(ShannonSubset$DayOfYear,ShannonSubset$Temp)
...
AICctab(SturgeonAbuGamma10,SturgeonAbuLog1,weights=T) 
summary(SturgeonAbuGamma10)
appraise(SturgeonAbuLog1)
appraise(SturgeonAbuGamma10)



SturgeonAbuGamma10$aic
##########33
#Top model
#########3
summary(SturgeonAbuGamma10)
gam.check(SturgeonAbuGamma10)
plot(SturgeonAbuGamma10)
appraise(SturgeonAbuGamma10) #Model ok at best, not great

cor(ShannonSubset$CTUSturgeon,ShannonSubset$Temp)


plot<-plot_smooth(SturgeonAbuGamma10, view="CTUSturgeon",rm.ranef=F,sim.ci = T)
plot_smooth(SturgeonAbuGamma10, view="temp_centered",rm.ranef=F,sim.ci = T)
#plot_sm
#plot$fv
FittedValues<-exp(plot$fv$fit)
head(FittedValues)
...
plot<-plot_smooth(SturgeonAbuGamma10, view="temp_centered",rm.ranef=F,sim.ci = T)
...
summary(InvertsByDischargeSampledDL3) #AICc weight = .691
summary(InvertsByDischargeSampledDL8) #AICc weight = .113 
...
#Second best model
summary(InvertsByDischargeSampledDL8)
intervals(InvertsByDischargeSampledDL8,which=c("fixed"))

#Intercept
exp(3.268941490) #26.28 estimate Intercept (centered)
exp(2.793188599) #16.33 CI
exp(3.744694381) #42.29 CI


(exp(-0.002595848)-1)*100 #-0.2592
(exp(-0.001576312)-1)*100 #-0.157% CI
(exp(-0.003615384)-1)*100 #-0.3608% CI
...
DOYInvertModelBoot<-ggplot(ShannonSubset,aes(DayOfYear,DriftInvertConc))+geom_point(color="darkgrey")+geom_line(data=pframe2,size=1.5)+  
  ...
  xlab("Day of Year")+ylab(expression(Macroinvertebrates~Per~100~m^3~Drift))+theme(axis.title.y=element_text(size = 11))
DOYInvertModelBoot
...
dev.off()
tiff("Figures/Fig3Jan2021.tiff", width = 174, height = 174, units = 'mm', res = 1200)
...
BiomassLog10 = lme(log(DriftBiomassConc)~CTUSturgeon+temp_centered,random=~1|Year,data=ShannonSubset,correlation=corAR1(form = ~DayOfYear|Year))


AICctab(BiomassLog0,BiomassLog1,BiomassLog2,BiomassLog3,BiomassLog4,BiomassLog5,BiomassLog6,BiomassLog7,BiomassLog8,BiomassLog9,BiomassLog10, weights=TRUE) 
summary(BiomassLog2)
summary(BiomassLog8)
...
intervals(BiomassLog8,which=c("fixed"))

#Intercept
exp(0.590381256) #1.8047 estimate intercept
exp(0.038868826) # 1.0396 CI
exp(1.141893687) # 3.133 CI

(exp(-0.003581698)-1)*100 #-0.357% estimate CTU
(exp(-0.004798236)-1)*100 #-0.479% CI
(exp(-0.002365159)-1)*100 #-0.236% CI
...
cor(ShannonSubset$CTUSturgeon,ShannonSubset$DoYCentered) #
cor(ShannonRichness$DayOfYear,ShannonRichness$percillum)
######################
#Biomass LMM plot
#####################
...
#Gamma models performing better but residual fit still off
# #NCrayfishs100
# #CrayfishConc
...
CrayfishAbuGamma5 <- gam((CrayfishPer100+10e-5)~ s(percillum)+s(Year,bs="re")+s(percillum), data = CrayfishAbu, method = "REML",correlation=corAR1(form = ~DayOfYear|Year),family= Gamma(link = "log"))
CrayfishAbuGamma6 <- gam((CrayfishPer100+10e-5)~ s(temp_centered)+s(Year,bs="re")+s(percillum), data = CrayfishAbu, method = "REML",correlation=corAR1(form = ~DayOfYear|Year),family= Gamma(link = "log"))
# 
AICctab(CrayfishAbuGamma0,CrayfishAbuGamma1,CrayfishAbuGamma2,CrayfishAbuGamma3,CrayfishAbuGamma4,CrayfishAbuGamma5,CrayfishAbuGamma6,weights=T)
...
SuckerDataFrame<-data.frame("Sucker",ShannonRichness$DPFS, ShannonRichness$DayOfYear,ShannonRichness$Year,ShannonRichness$PercentNightlyBiomassSucker)
...
PercentBiomassPlot<-ggplot(PlottingDataframe, aes(x=DayOfYear, y= PercentNightly,fill=Taxa))+facet_wrap(~Year)+geom_bar(stat="identity",lwd=0.1)+
  ...
  scale_fill_manual(values=cbPalette2)+scale_color_manual(values=cbPalette2)+theme(legend.text = element_text(size = 7),legend.title = element_blank())+ 
...
dev.off()
tiff("Figures/PercentBiomassPlotSuckerWeightsSeperated.tiff", width = 84, height = 84, units = 'mm', res = 1200)
PercentBiomassPlot
dev.off()
...
dev.off()
tiff("Figures/COISuckerCTU.tiff", width = 150, height = 150, units = 'mm', res = 1200)
SuckerCOICTU
dev.off()